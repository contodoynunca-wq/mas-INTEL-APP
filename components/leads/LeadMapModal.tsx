    ...
    useEffect(() => {
        ...
        const processLeads = async () => {
            ...
            const markerPromises = leadsToProcess.map(async (lead) => {
                ...
                if (!fullAddressForMap || fullAddressForMap.length < 3) {
                    // Try fallback to council + location if address is missing
                    if (lead.council) {
                        fullAddressForMap = `${lead.council}, ${job.location}`;
                    } else {
                ...
                try {
                    const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(fullAddressForMap)}&key=${API_KEY}&components=country:${countryCode}`);
                    const data = await response.json();
                    if (data.status === 'OK' && data.results[0]) {
                        return { status: 'OK', data: { ...data.results[0].geometry.location, lead } };
                    } else {
                        return { status: data.status, error_message: data.error_message || `Could not geocode address: ${fullAddressForMap}`, lead };
                    }
                } catch (error) {
