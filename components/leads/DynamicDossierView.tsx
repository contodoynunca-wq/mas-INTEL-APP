                ...
                <div className="space-y-6">
                    <div>
                        ...
                        {/* 
                            ...
                            1. Prioritize Static Map with Address if available (Most accurate for display/print)
                            2. Use Interactive Map only if coordinates are present
                            3. Show fallback if neither is reliable
                        */}
                        {(lead.address || lead.formattedAddress) ? (
                            <>
                                <div className="print-only">
                                    <StaticMap address={lead.formattedAddress || lead.address} />
                                </div>
                                {/* For interactive, we still need coords. If none, show static map instead. */}
                                {lead.geolocation ? (
                                    <div className="no-print">
                                        <InteractiveMap lat={lead.geolocation.lat} lng={lead.geolocation.lng} />
                                    </div>
                                ) : (
                                    <div className="no-print">
                                        <StaticMap address={lead.formattedAddress || lead.address} />
                            ...
                            <>
                                ...
                                <div className="print-only">
                                    <StaticMap lat={lead.geolocation.lat} lng={lead.geolocation.lng} />
                                </div>
                            </>
                        ) : (
                            <div className="p-4 bg-surface text-center rounded-lg text-text-secondary text-sm mb-2">
                         ...
                         <div className="flex flex-col gap-2 mt-4 no-print">
                            {lead.geolocation ? (
                                ...
                                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(lead.formattedAddress || lead.address || '')}`} target="_blank" rel="noopener noreferrer" className="btn tertiary w-full">Search Address on Google Maps</a>
                            )}
                        </div>
                    </div>
                     <div className="no-print">
