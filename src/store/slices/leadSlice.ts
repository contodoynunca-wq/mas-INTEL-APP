...
// --- HELPER: SAFE DATA MERGE ---
// Prevents overwriting valid data with empty strings or junk from AI
const safeMergeContactField = (currentValue: string | undefined, incomingValue: string | undefined | null): string | undefined => {
    // If incoming is empty/null/undefined, KEEP current
    if (!incomingValue || incomingValue.trim() === '' || incomingValue.toLowerCase() === 'n/a' || incomingValue.toLowerCase() === 'unknown') {
        return currentValue;
    }
    // If incoming looks like a placeholder, KEEP current
    if (incomingValue.includes('email.com') || incomingValue.includes('example.com')) {
        return currentValue;
    }
    // Otherwise, incoming data is likely better/newer
    ...
    fetchPrimaryData: async (options: { forceRefresh?: boolean } = {}) => {
        ...
        try {
            const collectionsToFetch = [
                { name: 'activeSearches', query: db.collection('activeSearches').where('userId', '==', currentUser.uid) },
                { name: 'savedLeads', query: db.collection('savedLeads').where('userId', '==', currentUser.uid) },
                { name: 'customerDirectory', query: db.collection('customers').where('userId', '==', currentUser.uid) },
                { name: 'projectPipeline', query: db.collection('projects').where('userId', '==', currentUser.uid) },
                { name: 'sentItems', query: db.collection('sentItems').where('userId', '==', currentUser.uid) },
                ...
                { name: 'internalContacts', query: db.collection('internalContacts') },
            ...
            incomingContacts.forEach(incoming => {
                ...
                if (matchIndex >= 0) {
                    ...
                    mergedContacts[matchIndex] = {
                        ...
                        ...incoming,
                        // SAFE MERGE LOGIC
                        email: safeMergeContactField(existing.email, incoming.email),
                        phone: safeMergeContactField(existing.phone, incoming.phone),
                        mobile: safeMergeContactField(existing.mobile, incoming.mobile),
                        linkedinUrl: safeMergeContactField(existing.linkedinUrl, incoming.linkedinUrl),
    ...
    verifyLeadContact: async (leadId, contactIndex) => {
        ...
        await processAiJob(async () => {
            ...
            newContacts[contactIndex] = {
                ...
                // SAFE MERGE: Only overwrite if result provides a valid non-empty string
                email: safeMergeContactField(contact.email, result.email),
                phone: safeMergeContactField(contact.phone, result.phone),
            };
            await updateLeadInJob(job.id, leadId, { companies: newContacts }, isSaved);
            ...
            for (let i = 0; i < contactsToProcess.length; i += BATCH_SIZE) {
                ...
                try {
                    ...
                    results.forEach(res => {
                         ...
                         // Safe merge for batch results
                         const existingLead = findParentJob(leadId).job?.leads.find(l => l.id === leadId);
                         const existingContact = existingLead?.companies[index];
                         
                         updatesByLead.get(leadId)!.set(index, {
                             status: res.status as any,
                             email: safeMergeContactField(existingContact?.email, res.email),
                             phone: safeMergeContactField(existingContact?.phone, res.phone),
                             mobile: safeMergeContactField(existingContact?.mobile, res.mobile)
                         });
                    });
                } catch (e) {
