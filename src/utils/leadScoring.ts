...
const calculateTimelinessScore = (lead: Lead): { score: number; details: string } => {
    ...
    let details: string = lead.projectStage || 'Unknown';
    const stage = (lead.projectStage || '').toLowerCase();
    const summary = (lead.summary || '').toLowerCase();

    // Check for "Watchlist" status from forensic audit
    if (stage.includes('watchlist') || stage.includes('stalled') || stage.includes('zombie')) {
        score = 0;
        details = 'Watchlist / Stalled';
    } else if (stage.includes('site') || stage.includes('construction') || stage.includes('started') || summary.includes('conditions discharged')) {
        score = 30;
