...
export const performForensicVerification = async (
    ...
    **CRITICAL ANTI-HALLUCINATION PROTOCOL (The 'Cross-Contamination' Check):**
    When scraping Council Agendas or Lists, LLMs often assign the Reference Number of Item A to the Location of Item B (e.g. Tregadillett gets the ref for St Ives).
    **VERIFICATION STEP:**
    For every Planning Ref found:
    1. Perform a specific search: 'Planning Application [Ref] Site Address'.
    2. IF the address returned does NOT match the Lead's location (e.g. Ref implies St Ives, Lead says Tregadillett), YOU MUST REJECT the Ref.
    3. Flag this as 'Cross-Contamination' in 'criticalAnomaly'.

    **TEMPORAL AND LOGIC CONSTRAINTS:**
    1. **Current Context Anchor:** The current date is **February 4, 2026**. Do not project dates forward.
    2. **Status Reconciler (Latency Check):** 
       ...
       - If the lead source says "Approved" (e.g. from 2024), you **MUST** search for "Construction Started [Project Name]" or "Discharge of Conditions" to see if it is now actually "On-Site". Fix the latency error.
      ...
      "results": [
        {
          ...
          "forensicReality": "string (The Truth - e.g., 'Actually just groundworks', 'Appeal Allowed', 'On Hold due to bankruptcy', 'Watchlist - Possible Stalled/Zombie Project')",
          "criticalAnomaly": "string | null (e.g., 'Cross-Contamination (Wrong Ref)', 'Fire Incident', 'Appeal Allowed', 'Infra-Only', 'Administration', 'Sequential Ref Rejected')",
          "strategicAction": "'Monitor' | 'Pitch' | 'Discard'",
          "newProjectStage": "string (Suggested updated stage, e.g. 'On-Site', 'Pre-Construction', 'Overdue / Stalled', 'Watchlist')",
    ...
    try {
        ...
        const response = await executeRequest(ai, {
            ...
            config: {
                ...
                // thinkingConfig removed to avoid invalid parameter errors
            },
            signal
        });
...
import { getMultimodalEmbedding } from '../../../services/ai/embeddingService';
