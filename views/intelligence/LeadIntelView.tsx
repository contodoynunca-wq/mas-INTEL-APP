...
import { generateAIStrategyForLocation, generateOutreachEmail } from '../../services/ai/leadIntelService';
...
const LeadIntelView: FC = () => {
    ...
    const [searchMode, setSearchMode] = useState<'smart' | 'global'>('smart');
    ...
    const currentLeads = useMemo(() => {
        if (isGlobalSearchActive && globalLeadSearchQuery) {
    ...
    const handleBulkAction = async (action: string) => {
        ...
        if (action === 'forensicVerify') { 
            ...
            // Only proceed if res is defined (successful API call)
            if (res) {
                if (res.length > 0) {
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-shrink-0 bg-bg-secondary border-b border-border-color p-3 flex flex-col gap-2 shadow-sm z-10">
                <div className="flex items-center gap-3 w-full">
                    ...
                    <form onSubmit={handleSearchSubmit} className="flex-grow flex items-center relative min-w-0">
                        <input 
                            type="text" 
                            value={searchMode === 'smart' ? smartSearchQuery : localGlobalSearch}
                            onChange={e => searchMode === 'smart' ? setSmartSearchQuery(e.target.value) : setLocalGlobalSearch(e.target.value)}
                            ...
                            placeholder={searchMode === 'smart' ? "e.g. 'Roofers in Bristol' or 'New Builds in Cornwall'..." : "Search database by name, company, or address..."}
                        />
